...
Feature: Logout
  ...
  Scenario: Logout returns 200 with success message
    ...
    And Assert response body contains "Logged out" in "response"
  ...
  Scenario: Logout without token returns 401
    Given Create minimal user and save response as "createRes"
    And Extract "id" from "createRes" and save as "userId"
    When Logout user "userId" with token "fake_token" and save response as "response"
  ...
  @Run @Negative @allure.label.story:Negative_Scenario
  Scenario: Logout non-existent user returns 401
